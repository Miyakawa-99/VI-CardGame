...
final class ChatViewController: UIViewController{
  ...
  //ここなんかフィードバック欲しい
    ...
    @IBAction func Stop(_ sender: Any) {
        sessionContainer.sendMessage("miss")
    }
  ...
  ///今ある札確認するよー
    ...
    func insertTranscript(_ transcript: Transcript) {
        ...
        case "0","1","2","3","4","5","6","7":
            ...
            self.sessionContainer.sendMessage("stop") 
            ...
            DispatchQueue.main.async {
                ...
                if let name = self.TappedCard {
                 if(self.TappedCard  == self.CorrectCard){
                    //String(self.CorrectCard) + "flag" = true
                   ...
                   /* if(self.Card1.currentTitle == self.CorrectCard){
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
                         self.sessionContainer.sendMessage("finish")
                    }
                  
                }
              else {//入らない
                    print("else")
                         print(self.card[self.Karuta.conversion(SoundName: self.CorrectCard!)])
                    if(self.card[self.Karuta.conversion(SoundName: self.CorrectCard!)] == 1){
                        self.sound.playSound(name: "Wrong")
                        self.sound.play()
                        //self.sessionContainer.sendMessage("miss")
